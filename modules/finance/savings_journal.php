...
<nav class="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-6 z-20 shadow-sm relative">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-slate-800 leading-none">Jurnal Tabungan</h1>
            <span class="text-xs text-slate-500 font-medium">Laporan Transaksi Simpanan Siswa</span>
        </div>
    </div>
</nav>
...
<main class="flex-1 overflow-hidden flex flex-col bg-slate-50 relative">
    ...
    <div class="bg-white p-4 border-b border-slate-200 flex flex-wrap gap-4 items-end shadow-sm z-10">
        <div>
            ...
            <input type="date" v-model="filter.startDate" class="border rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500">
        </div>
        <div>
            <label class="block text-xs font-bold text-slate-500 mb-1">Tanggal Akhir</label>
            <input type="date" v-model="filter.endDate" class="border rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500">
        </div>
        <div>
            <label class="block text-xs font-bold text-slate-500 mb-1">Unit</label>
            <select v-model="filter.unitPrefix" class="border rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500 min-w-[150px]">
                <option value="">Semua Unit</option>
                <option v-for="u in units" :key="u.id" :value="u.receipt_code">{{ u.receipt_code }}</option>
            </select>
        </div>
        <button @click="fetchData" class="bg-blue-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-blue-700 text-sm shadow-lg shadow-blue-200 h-[38px]">
                ...
                <tbody class="divide-y divide-slate-100">
                    <tr v-for="(item, index) in list" :key="item.id" class="hover:bg-slate-50 transition-colors align-top">
                        ...
                        <td class="px-6 py-4">
                            ...
                            <div class="flex gap-4 text-xs text-slate-500 mt-2">
                                <div v-if="item.transaction_type">
                                    <span class="font-bold">Tipe:</span> 
                                    <span :class="item.transaction_type === 'DEPOSIT' ? 'text-green-600' : 'text-red-600'" class="font-bold">
                                        {{ item.transaction_type === 'DEPOSIT' ? 'SETORAN' : 'PENARIKAN' }}
                                    </span>
                                </div>
                                <div v-if="item.unit_code">
                                    <span class="font-bold">Unit:</span> {{ item.unit_code }}
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-0 py-0">
                            <table class="w-full">
                                <tbody class="divide-y divide-slate-50">
                                    <tr v-for="j in item.items" :key="j.code" class="bg-slate-50/50">
                    ...
                    <tr v-if="list.length === 0">
                        <td colspan="4" class="px-6 py-12 text-center text-slate-400 italic">
                            Belum ada jurnal tabungan pada periode ini.
                        </td>
                    </tr>
                </tbody>
            </table>
...
<script>
    ...
    createApp({
        ...
        methods: {
            ...
            async fetchData() {
                try {
                    const res = await fetch(`${window.BASE_URL}api/finance.php?action=get_savings_journals&start_date=${this.filter.startDate}&end_date=${this.filter.endDate}&unit_prefix=${this.filter.unitPrefix}`);
                    const data = await res.json();
                    if (data.success) {
                        this.list = data.data;
                    }
