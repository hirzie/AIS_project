...
<nav class="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-6 z-20 shadow-sm relative">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-slate-800 leading-none">Jurnal Penerimaan</h1>
            <span class="text-xs text-slate-500 font-medium">Laporan Transaksi Pendapatan</span>
        </div>
    </div>
</nav>
...
<main class="flex-1 overflow-hidden flex flex-col bg-slate-50 relative">
    ...
    <div class="bg-white p-4 border-b border-slate-200 flex flex-wrap gap-4 items-end shadow-sm z-10">
        <div>
            ...
            <input type="date" v-model="filter.startDate" class="border rounded px-3 py-2 text-sm focus:outline-none focus:border-green-500">
        </div>
        <div>
            <label class="block text-xs font-bold text-slate-500 mb-1">Tanggal Akhir</label>
            <input type="date" v-model="filter.endDate" class="border rounded px-3 py-2 text-sm focus:outline-none focus:border-green-500">
        </div>
        <div>
            <label class="block text-xs font-bold text-slate-500 mb-1">Unit</label>
            <select v-model="filter.unitPrefix" class="border rounded px-3 py-2 text-sm focus:outline-none focus:border-green-500 min-w-[150px]">
                <option value="">Semua Unit</option>
                <option v-for="u in units" :key="u.id" :value="u.receipt_code">{{ u.receipt_code }}</option>
            </select>
        </div>
        <button @click="fetchData" class="bg-green-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-green-700 text-sm shadow-lg shadow-green-200 h-[38px]">
                ...
                <tbody class="divide-y divide-slate-100">
                        ...
                        <td class="px-6 py-4">
                            ...
                            <div class="flex gap-4 text-xs text-slate-500 mt-2">
                                ...
                                <div v-if="item.receiver">
                                    <span class="font-bold">Diterima:</span> {{ item.receiver || '-' }}
                                </div>
                            </div>
                        </td>
                    ...
                    <tr v-if="list.length === 0">
                        <td colspan="4" class="px-6 py-12 text-center text-slate-400 italic">
                            Belum ada jurnal penerimaan pada periode ini.
                        </td>
                    </tr>
                </tbody>
            </table>
...
<script>
    ...
    createApp({
        ...
        methods: {
            ...
            async fetchData() {
                try {
                    const res = await fetch(`${window.BASE_URL}api/finance.php?action=get_income_journals&start_date=${this.filter.startDate}&end_date=${this.filter.endDate}&unit_prefix=${this.filter.unitPrefix}`);
                    const data = await res.json();
                    if (data.success) {
                        this.list = data.data;
                    }
