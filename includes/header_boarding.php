...
<nav class="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-6 z-20 shadow-sm relative flex-none">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-slate-800 leading-none">Manajemen Asrama</h1>
            <span class="text-xs text-slate-500 font-medium">Boarding System Portal</span>
        </div>
    </div>
    ...
    <div class="flex items-center gap-4">
        ...
        <div class="flex items-center bg-slate-100 rounded-lg p-1 border border-slate-200">
            <button @click="currentGender = 'all'" 
                    :class="{'bg-white shadow text-indigo-600 font-bold': currentGender === 'all', 'text-slate-500 hover:text-slate-700': currentGender !== 'all'}" 
                    class="px-4 py-1.5 rounded-md text-xs transition-all">
                Semua
            </button>
            <button @click="currentGender = 'L'" 
                    :class="{'bg-white shadow text-indigo-600 font-bold': currentGender === 'L', 'text-slate-500 hover:text-slate-700': currentGender !== 'L'}" 
                    class="px-4 py-1.5 rounded-md text-xs transition-all">
                Ikhwan (L)
            </button>
            <button @click="currentGender = 'P'" 
                    :class="{'bg-white shadow text-indigo-600 font-bold': currentGender === 'P', 'text-slate-500 hover:text-slate-700': currentGender !== 'P'}" 
                    class="px-4 py-1.5 rounded-md text-xs transition-all">
                Akhwat (P)
            </button>
        </div>
